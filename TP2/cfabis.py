...
def demat_bilin(img_name, cfa="GRBG", show=False, write= False):
    ...
    convolutionG = est_G_ha(cfa_img)
    convolutionG = convolutionG.astype(convolutionR.dtype)
    """ plt.imshow(convolutionG)
    plt.show() """
    print(convolutionR.shape,convolutionG.shape,convolutionR.shape)
    print(convolutionR.dtype,convolutionG.dtype,convolutionB.dtype)
...
def est_G_ha(cfa_img, cfa="GRBG"):
    ...
    for i, j in indices_blue:
        gradientX = gradientBlueX[i, j]
        gradientY = gradientBlueY[i, j]

        if gradientX > gradientY and j < cfa_img.shape[1]/2-2 and i < cfa_img.shape[0]/2-2:
                newImg[i * 2+1, j * 2] = (cfa_img[i * 2, (j - 1) * 2] + cfa_img[i * 2, (j + 1) * 2]) / 2 \
                                                + (2 * cfa_img[i * 2, j * 2] - cfa_img[i * 2, (j - 2) * 2] - cfa_img[i * 2, (j + 2) * 2]) / 4

        elif gradientY > gradientX and i < cfa_img.shape[0]/2-2:
                newImg[i * 2+1, j * 2] = (cfa_img[(i - 1) * 2, j * 2] + cfa_img[(i + 1) * 2, j * 2]) / 2 \
                                                + (2 * cfa_img[i * 2, j * 2] - cfa_img[(i - 2) * 2, j * 2] - cfa_img[(i + 2) * 2, j * 2]) / 4

        elif j < cfa_img.shape[1]/2-2 and i < cfa_img.shape[0]/2-2:
                newImg[i * 2+1, j * 2] = (cfa_img[(i - 1) * 2, (j - 1) * 2] + cfa_img[(i - 1) * 2, (j + 1) * 2] +
                                                cfa_img[(i + 1) * 2, (j - 1) * 2] + cfa_img[(i + 1) * 2, (j + 1) * 2]) / 4 \
                                                + (2 * cfa_img[i * 2, j * 2] -
                                                cfa_img[(i - 2) * 2, (j - 2) * 2] - cfa_img[(i - 2) * 2, (j + 2) * 2] -
                                                cfa_img[(i + 2) * 2, (j - 2) * 2] - cfa_img[(i + 2) * 2, (j + 2) * 2]) / 8
    indices_red = np.column_stack(np.where(cfa_img[1::2, 0::2]))
    for i, j in indices_red:
        gradientX = gradientRedX[i, j]
        gradientY = gradientRedY[i, j]

        if gradientX > gradientY and j < cfa_img.shape[1]/2-2 and i < cfa_img.shape[0]/2-2:
            newImg[i * 2, j * 2 + 1] = (cfa_img[i * 2, (j - 1) * 2 + 1] + cfa_img[i * 2, (j + 1) * 2 + 1]) / 2 \
                                        + (2 * cfa_img[i * 2, j * 2 + 1] - cfa_img[i * 2, (j - 2) * 2 + 1] - cfa_img[i * 2, (j + 2) * 2 + 1]) / 4

        elif gradientY > gradientX and i < cfa_img.shape[0]/2-2:
            newImg[i * 2, j * 2 + 1] = (cfa_img[(i - 1) * 2, j * 2 + 1] + cfa_img[(i + 1) * 2, j * 2 + 1]) / 2 \
                                        + (2 * cfa_img[i * 2, j * 2 + 1] - cfa_img[(i - 2) * 2, j * 2 + 1] - cfa_img[(i + 2) * 2, j * 2 + 1]) / 4

        elif j < cfa_img.shape[1]/2-2 and i < cfa_img.shape[0]/2-2:
            newImg[i * 2, j * 2 + 1] = (cfa_img[(i - 1) * 2, (j - 1) * 2 + 1] + cfa_img[(i - 1) * 2, (j + 1) * 2 + 1] +
                                        cfa_img[(i + 1) * 2, (j - 1) * 2 + 1] + cfa_img[(i + 1) * 2, (j + 1) * 2 + 1]) / 4 \
                                        + (2 * cfa_img[i * 2, j * 2 + 1] -
                                        cfa_img[(i - 2) * 2, (j - 2) * 2 + 1] - cfa_img[(i - 2) * 2, (j + 2) * 2 + 1] -
                                        cfa_img[(i + 2) * 2, (j - 2) * 2 + 1] - cfa_img[(i + 2) * 2, (j + 2) * 2 + 1]) / 8
    return newImg
